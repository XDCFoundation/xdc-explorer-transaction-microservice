...
export default class Index {
    ...
    async getCoinMarketDetailForTransaction(request, response) {
        Utils.lhtLog('Inside getCoinMarketDetailForTransaction', request.params, 'getCoinMarketDetailForTransaction', 0, '')
        const [error, coinMarketDetailForTransactionResponse] = await Utils.parseResponse(new BLManager().getCoinMarketDetailForTransaction(request.query))
        if (!coinMarketDetailForTransactionResponse) {
            return Utils.handleError(error, request, response)
        }
        return Utils.response(response, coinMarketDetailForTransactionResponse, apiSuccessMessage.FETCH_SUCCESS, httpConstants.RESPONSE_STATUS.SUCCESS, httpConstants.RESPONSE_CODES.OK)
    }
}
